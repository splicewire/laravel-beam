...
class BeamInstallTest extends TestCase
{
    ...
    public function test_beam_core_self_registers_its_own_step_core_first(): void
    {
        ...
        $this->assertNotContains('beam-scribe', $core->publishTags);
        $this->assertSame([], $core->commands);
    ...
    public function test_completion_commands_run_only_for_selected_packages(): void
    {
        $ran = [];
        Artisan::command('fixture:complete-wanted', function () use (&$ran): void {
            $ran[] = 'wanted';
        });
        Artisan::command('fixture:complete-skipped', function () use (&$ran): void {
            $ran[] = 'skipped';
        });
        $manifest = $this->app->make(BeamInstallManifest::class);
        $manifest->register('acme/wanted', commands: ['fixture:complete-wanted']);
        $manifest->register('acme/skipped', commands: ['fixture:complete-skipped']);

        $this->artisan('splicewire:beam:install', ['--modules' => 'wanted', '--no-interaction' => true])
            ->assertExitCode(0);

        $this->assertSame(['wanted'], $ran);
    }

    public function test_a_failed_completion_command_makes_the_install_fail(): void
    {
        Artisan::command('fixture:fail-completion', fn () => 7);
        $this->app->make(BeamInstallManifest::class)->register(
            'acme/failing', commands: ['fixture:fail-completion'],
        );

        $this->artisan('splicewire:beam:install', ['--no-interaction' => true])
            ->expectsOutputToContain('One or more package completion commands failed')
            ->assertExitCode(1);
    }

...
class PersistsBeamParticleTest extends TestCase
{
    ...
    {
        $a = BeamParticleFixture::create(['schema_ref' => 'x/1', 'payload' => ['a' => 1]]);
        $b = BeamParticleFixture::create(['schema_ref' => 'x/2', 'payload' => ['a' => 2]]);
    ...
    {
        $r = BeamParticleFixture::create([
            'schema_ref' => 'x/1',
            'payload' => ['nested' => ['k' => 'v']],
            'meta' => ['derived' => true],
        ]);

        $fresh = BeamParticleFixture::find($r->id);
    ...
    {
        $r = new BeamParticleFixture;
        $this->assertNull($r->extract());
    }
}

class BeamParticleFixture extends Model
{

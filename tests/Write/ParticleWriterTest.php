 ...
 * {@see ParticleWriter} entry point and observes only what a caller observes — a record persisted or
 * refused, an event emitted — never the pipeline's internal step ordering (DESIGN "Testing Decisions").
 *
 * It is exercised against BOTH a base {@see BeamParticle} (schema-validated: its content lives in the
...
class ParticleWriterTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        // the shipped default target resolver resolves them for the BeamParticle validation path.
    ...
    public function test_a_conforming_payload_persists_and_emits_the_event(): void
    {
        ...
        $record = $this->writer()->write(new BeamParticle(['schema_ref' => self::CHEAP_REF]), $payload);
        ...
        $this->assertSame($payload, BeamParticle::findOrFail($record->id)->payload);

        // One post-persist signal, carrying the record, the payload, and the binding.
        Event::assertDispatched(BeamParticlePersisted::class, function (BeamParticlePersisted $e) use ($record, $payload) {
    ...
    public function test_a_non_conforming_payload_is_refused_and_nothing_persists(): void
    {
        ...
        try {
            ...
            $this->writer()->write(new BeamParticle(['schema_ref' => self::CHEAP_REF]), ['body' => 'no title']);
        ...
        Event::assertNotDispatched(BeamParticlePersisted::class);
    }
    ...
    public function test_the_writer_is_model_agnostic_across_app_model_and_base_record(): void
    {
        ...
        // The base BeamParticle — schema-validated content in the payload column, versioned binding.
        $content = ['title' => 'Rec', 'body' => null, 'summary' => null];
        $record = $this->writer()->write(new BeamParticle(['schema_ref' => self::CHEAP_REF]), $content);
        $this->assertDatabaseHas(Beam::table('particles'), ['id' => $record->id, 'schema_ref' => self::CHEAP_REF]);
        $this->assertSame($content, BeamParticle::findOrFail($record->id)->payload);
    }
    ...
    private function writer(): ParticleWriter
    {
        return $this->app->make(ParticleWriter::class);
    }
...
/**
 * A representative app model riding the pipeline: it uses {@see PersistsBeamParticle} on its OWN table

 ...
 * the whole hydrator (the coarser read seam stays the {@see ParticleHydrator} port). A source-aware
 * hydrator can replace that port. Passing no `$stages` runs the shipped single-stage chain.
 *
    ...
    public function hydrate(Model|array|string $source, ReadContext $ctx): Data
    {
        if (! $source instanceof Model) {
            throw new InvalidArgumentException(
                'The payload reader hydrates a persisted record; pass a Model (a source-aware hydrator handles refs/arrays).',
            );
        }
